...
export class TournamentHistoryPage extends BaseComponent {
    constructor(){
        super("static/html/tournament_history.html")
    }
    ...
    async onInit(){       
       ...
       indexInput1.addEventListener('input', (e) => {
           ...
           if (value < 0 || value > this.tournaments) {
               e.target.value = 0;
           }
       });
               ...
               if (response.ok) {
                   ...
                   document.getElementById('message').innerHTML = `
                       <div class="alert" style="color: #ff4444; text-shadow: 0 0 10px #ff4444; border: 2px solid #ff4444; padding: 1rem; margin-top: 1rem;">
                           <p>Search Out of Bounds, check the Number of Tournaments</p>
                       </div>
                   `;
               }
           } catch (error) {
